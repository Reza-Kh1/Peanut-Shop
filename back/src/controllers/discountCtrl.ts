...
const createDiscount = asyncHandler(async (req, res: Response) => {
    ...
    try {
        await prisma.discount.create({
            data: {
                amount: amount || undefined, code: code || undefined, type, discount, startDate, endDate
            }
        })
...
const checkDiscount = asyncHandler(async (req, res: Response) => {
    ...
    try {
        ...
        if (data?.amount) {
            res.status(404).json({ message: "تعداد استفاده از این کد تخفیف به پایان رسیده است" });
            return
        }
        if (!data) {
