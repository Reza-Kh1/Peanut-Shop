...
const signUp = asyncHandler(async (req, res: Response) => {
    ...
    try {
        ...
        let data;
        if (!admin) {
            data = await prisma.user.create({ data: { name, email, password: hash, phone, role: "ADMIN" } })

        } else {
            data = await prisma.user.create({ data: { name, email, password: hash, phone } })
        }
        data.password = ""
        if (data.role !== "CUSTOMER") {
            const token = createToken(data)
            res.cookie("peanutUser", token, {
                httpOnly: true,
                secure: true,
                sameSite: "strict",
...
const forgetPassword = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        await prisma.user.update({ where: { email }, data: { password: await createHash(newPass) } })
