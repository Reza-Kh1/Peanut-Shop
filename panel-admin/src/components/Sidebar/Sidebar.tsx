...
export default function Sidebar() {
  ...
  const NavlinkAdmin = ({ name, href, icon }: NavlinkAdminType) => {
    return <Link to={href} className={`${pathname.startsWith(href) ? "bg-white shadow-md" : null} flex items-center p-2 rounded-md  justify-strat gap-2`}>
  ...
  return (
    <div className='hidden md:flex w-2/12 sticky left-0 top-5 h-full flex-col gap-2'>
      ...
      <div className='h-[calc(100vh-110px)] pr-2 overflow-y-auto custom-scroll flex flex-col gap-1'>
        ...
        <NavlinkAdmin href='/admin/payment' icon={<FaFileInvoiceDollar />} name='سبد خرید' />
